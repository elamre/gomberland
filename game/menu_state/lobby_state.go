...
func newlobbyState() *lobbyState {
	...
	tOpts := []widget.TextInputOpt{
		...
		widget.TextInputOpts.Image(assets.GetManager().AssetMap[assets.GenericInputImage].(*widget.TextInputImage)),
		widget.TextInputOpts.Color(assets.GetManager().AssetMap[assets.GenericInputColor].(*widget.TextInputColor)),
		...
		widget.TextInputOpts.Face(fontAsset),
		widget.TextInputOpts.CaretOpts(
			widget.CaretOpts.Size(fontAsset, 2),
		),
	}
...
func (m *lobbyState) GetState() string {
	return m.curState
}

func (m *lobbyState) Init() any {
	return nil
}

func (m *lobbyState) Update() {
	m.ui.Update()
}

func (m *lobbyState) Draw(game *tentsuyu.Game) {
	m.ui.Draw(game.Screen)
}

func (m *lobbyState) DeInit() any {
	return nil
}
